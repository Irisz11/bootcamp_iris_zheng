...
This homework implements a reproducible storage layer using environment-driven paths defined in `.env`.
...
The paths are configured using:
...
The notebook creates the required directories automatically if they do not already exist.
...
Reusable utility functions are also implemented to automatically detect file format from the file suffix and route read/write operations to CSV or Parquet.
...
These checks help detect storage-related type drift or structural changes.

Assumptions and Risks
A Parquet engine such as pyarrow or fastparquet must be installed for Parquet read/write operations.
